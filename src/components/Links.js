...
const Links = () => {
    ...
    const fetchInitialData = async () => {
      try {
        ...
        const cardQuerySnapshot = await getDocs(
          query(collection(db, "cardsforLinks"), orderBy("createdAt", "asc"))
        );
        ...
        const categoryQuerySnapshot = await getDocs(
          collection(db, "categoriesforLinks")
        );
  ...
  const saveCategoryToFirestore = async (category) => {
    try {
      await addDoc(collection(db, "categoriesforLinks"), { name: category });
  ...
  const addCard = async () => {
    ...
    try {
      const docRef = await addDoc(collection(db, "cardsforLinks"), newCard);
      newCard.id = docRef.id;

      // "updates" コレクションに履歴を追加
      const updatesDoc = {
  ...
  const deleteCategoryFromFirestore = async (categoryToDelete) => {
    try {
      ...
      if (categoryDoc) {
        // 一致したドキュメントを削除
        await deleteDoc(doc(db, "categoriesforLinks", categoryDoc.id));
  ...
  const updateCardInFirestore = async (id, updatedCard) => {
    try {
      const cardRef = doc(db, "cardsforLinks", id);
