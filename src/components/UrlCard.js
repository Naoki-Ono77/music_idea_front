...
const UrlCard = ({ card, categories, onUpdate, onDelete }) => {
  ...
  const saveChanges = () => {
    console.log({ id: card.id, title, url: card.url, text, category, comments:[...card.comments] }); // 渡すオブジェクトを確認
    onUpdate({ id: card.id, title, url: card.url, text, category, comments: [...card.comments] });
    setIsEditing(false);
  };

  const addComment = () => {
    if (comment.trim()) {
      onUpdate({id: card.id, title, url: card.url, text, category, comments: [...card.comments, comment]});
      setComment("");
    }
  };
  ...
  const deleteComment = (index) => {
    ...
    onUpdate( { id: card.id,  title, url: card.url, text, category, comments: updatedComments });
  };
    ...
    <div className="card">
      ...
      <div className="card-actions">
        ...
        <button onClick={() => setIsCommentsVisible(!isCommentsVisible)}>
          <FaCommentDots />
          {card.comments.length > 0 && <span className="comments-count">{card.comments.length}</span>}
        </button>
      </div>
